...
class _FluEyeScreenState extends State<FluEyeScreen>{
  ...
  Future<void> _tfLteInit() async {
    await Tflite.loadModel(
        ...
        isAsset:
        true, // modelin assets klasöründen yüklendiğini belirtir
        useGpuDelegate:
        false // true -> gpu , false -> cpu
  ...
  pickImageGallery () async {
    ...
    final XFile? image = await picker.pickImage(source: ImageSource.gallery); //galeriden resim seçilmesi ve de pathi tutar
  ...
  pickImageCamera() async {
    final ImagePicker picker = ImagePicker(); //imagepickerden nesne oluşturuldu
    final XFile? image = await picker.pickImage(source: ImageSource.camera); //cameradan resim çekilmesi ve de pathi tutar
              ...
              Column(
                children: [
                  ElevatedButton.icon(
                    ...
                    icon: const Icon(Icons.image_outlined, size: 24.0, color: Colors.red),
                    label: const Padding(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      disabledForegroundColor: Colors.red.shade700.withOpacity(0.38),
                      disabledBackgroundColor: Colors.red.shade700.withOpacity(0.12),
                  ...
                  ElevatedButton.icon(
                    ...
                    icon: const Icon(Icons.camera_alt_outlined, size: 24.0, color: Colors.red),
                    label: const Padding(
                      padding: EdgeInsets.symmetric(vertical: 12.0, horizontal: 10.0),
                      child: Text(
                        "Take Photo",
                  ...
                  ElevatedButton.icon(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => FluInfoScreen()),
                      );
                    },
                    icon: const Icon(Icons.info_outline, size: 24.0, color: Colors.red),
                    label: const Padding(
                      padding: EdgeInsets.symmetric(vertical: 12.0, horizontal: 10.0),
                      child: Text(
                        "Get Information",
